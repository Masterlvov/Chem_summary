...
在进行电位弛豫测量之前，电池先进行预循环，然后以0.05 C的倍率放电/充电至某一电位。移除电流后，记录48小时内开路电压（OCV，φ）随时间（t）的变化。将1小时内2 mV范围内的初始电位变化设定为平衡电位（即弛豫时间趋于无穷大时的电极电位，φ∞）。电压先逐渐升高/降低，然后趋于稳定。快速的电压变化归因于电解液和短程界面中的Li+扩散，而随后的缓慢变化则归因于Li+在长程颗粒和电极中的扩散。
 ...
 商用片状石墨的平均粒径为6 μm，制备了面载量约为2.5 mg cm⁻²的薄电极且未经过辊压，以便在最小化电极内部扩散距离的同时，比较界面和颗粒间的Li⁺扩散动力学。该电极分别在低电流密度和高电流密度（0.05 C与0.5 C，1 C = 372 mA g⁻¹，图2）下进行放电。将Li⁺嵌入速率提高一个数量级会显著增加电池极化约70 mV，因此嵌入反应在最后一个平台之前停止，导致初始容量较低（图2a）。在相同嵌锂含量（250 mAh g⁻¹）下，通过XRD对比了嵌锂石墨的结构。在0.5 C下观察到四个阶结构的共存，而在0.05 C下仅存在单相II阶结构（图2b），表明不同阶结构之间的转变受反应动力学限制，快速的Li⁺嵌入导致Li⁺分布不均匀。这导致快速Li⁺嵌入时嵌锂石墨的长程结构非均一性。0.5 C下的原位XRD图谱（图S1）进一步证实了由于缓慢的反应动力学，不同阶结构共存且转变不完全。
[石墨负极嵌锂动力学](https://link.springer.com/article/10.1007/s40820-023-01183-6/figures/2)


当活性物质载量、电极厚度和压实密度增加时，Li⁺在电极中的扩散动力学变得不可忽略，甚至主导了石墨的倍率性能（图6a）。将载量从1.9增加到4.6和6.1 mg cm⁻²，会显著降低高倍率下的可逆容量，因为Li⁺的垂直扩散长度分别从35.52延长至80.34和113.17 μm（图6b–c）。压延后，尽管电极厚度缩减至不足一半（图6b），但电极压实密度大幅提高（图6b），导致容量进一步下降（图6a）。当活性物质载量增加时，Li⁺扩散长度相应增加，厚电极需要更长时间才能达到平衡电位（图S10），对应1.9、4.6和6.1 mg cm⁻²分别为2.2 × 10⁴、2.5 × 10⁴和3.2 × 10⁴ s。这些结果表明，对于实际应用中的高载量、高压实石墨电极，电极工程对于促进Li⁺在电极中的扩散至关重要，例如垂直取向石墨电极[38, 39]和激光图案化[40]。与传统电解液相比，含FEC电解液的厚石墨电极表现出更优异的倍率性能（图S11），表明电解液调控或界面设计对于促进Li⁺快速嵌入石墨也是必要的。



快速Li⁺嵌入石墨受到Li⁺在石墨颗粒、界面和电极中扩散的限制。基于实验测得的颗粒内Li⁺扩散系数（> 2.64 × 10⁻¹⁰ cm² s⁻¹），10 μm石墨能够以6 C倍率快速充电，而20 μm石墨则降至2 C（图4f和S12）。当使用6 μm石墨片时，对于薄石墨电极（< 2 mg cm⁻²），速率决定步骤受界面Li⁺扩散控制，包括去溶剂化和穿过SEI层的过程，这高度依赖于电解液化学性质。构建氟化SEI层被证明有利于加速界面Li⁺扩散，并显著提升石墨的快充性能。对于厚而致密的石墨电极，电极中的Li⁺扩散成为主要反应决定步骤。因此，为实现高性能快充锂离子电池石墨负极，应充分考虑并精心设计匹配石墨颗粒尺寸、电解液、界面和电极构型。
